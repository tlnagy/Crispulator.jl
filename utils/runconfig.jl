...
function runconfig(setup::ScreenSetup,
    ...
    gen_plots = (bc_counts, genes) -> begin
        ...
        Theme(highlight_width=0mm), Coord.cartesian(fixed=true),
    ...
    snr_result = "SNR score = $(round(mean_snr, digits=3)) +/- $(round(std_snr, digits=3))"
    select!(results, Not([:x19, :x20]))
    hierarchy = vcat([hcat(item...) for item in Iterators.product(map(Symbol, methods), measures, genetypes)]...)
    new_names = [[:method, :measure, :genetype, :score]...; fieldnames(typeof(setup))...; :run_idx]
    results = construct_hierarchical_label(hierarchy, results, new_names)

    venn_result, auprc_result = "", ""

    grouped_info = combine(DataFrames.groupby(results, [:method, :measure, :genetype])) do grouped_df
        n = size(grouped_df, 1)
        ...
        if (grouped_df[1, :measure] == :incdec) &&
           (grouped_df[1, :genetype] == :all)
            ...
            disp_score = round(mean_score, digits=3)
            conf_max = round(mean_score + conf_int, digits=3)
            conf_min = round(mean_score - conf_int, digits=3)
